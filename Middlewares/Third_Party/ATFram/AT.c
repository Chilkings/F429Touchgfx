...
void ATCommandSendScheduler(void)
{
  ...
  if(xResult != pdPASS)
  {
	...
	xSemaphoreTake(ATSchRunSemaphore,portMAX_DELAY);/* 获得互斥量: 上锁 */
  ...
  {	//发送失败后重启
  	...
  	{//发送失败后重试
  		...
  		switch((int)SendATInfo.RegCommandType)
			...
			osDelay(200);  //等待模块回复，可按情况调整，不加此延时容易接收不全
